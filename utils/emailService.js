...
// Create transporter using Brevo SMTP (not Gmail)
const transporter = nodemailer.createTransport({
...
transporter.verify((error, success) => {
  if (error) {
    ...
    console.log("✅ Email service ready to send messages (Brevo)");
  }
});
...
export const sendEmail = async (to, subject, html) => {
  try {
    const info = await transporter.sendMail({
      from: `"Your App" <${process.env.BREVO_USER}>`,  // Use BREVO_USER as sender
      to: to,
      subject: subject,
      html: html,
    });
